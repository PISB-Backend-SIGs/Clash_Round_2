...
def execute_python_code():
    try:
        process = subprocess.Popen(f"docker exec {CONTAINER_NAME} sh -c 'timeout 5s python3 src/code.py < src/input.txt'",stdout=subprocess.PIPE, stderr=subprocess.PIPE,preexec_fn=set_time_limit(TimeoutLimit),shell=True)
